...
latest_model_stats as (
    select
        ...
        {% endif %}
        , max(case when not was_full_refresh {% if target.type == 'sqlserver' %}= 1{% endif %} then query_completed_at end) as last_incremental_run_completed_at
        , max(case when not was_full_refresh {% if target.type == 'sqlserver' %}= 1{% endif %} then total_node_runtime end) as last_incremental_run_total_runtime
        , max(case when not was_full_refresh {% if target.type == 'sqlserver' %}= 1{% endif %} then rows_affected end) as last_incremental_run_rows_affected
        {% if target.type == 'bigquery' %}
